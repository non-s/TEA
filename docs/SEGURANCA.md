...
**Limitação conhecida**: esta funcionalidade não tem testes automatizados de regras em emulador (o projeto não tem infraestrutura de emulator/rules-unit-testing configurada). Em compensação, o fluxo completo foi verificado manualmente em produção em julho de 2026, com contas reais e e-mail de verificação de verdade: (1) colaborador com e-mail ainda não verificado tenta abrir o link e é recusado, com a mensagem de erro esperada; (2) depois de clicar no link de verificação recebido por e-mail e refazer login (para obter um token novo com `email_verified: true`), o mesmo colaborador consegue ler o progresso e registrar uma observação de sessão; (3) uma terceira conta, sem o e-mail na lista de colaboradores, é recusada mesmo criando conta nova. Os três testes bateram com o comportamento esperado das regras.

Achado da verificação: o e-mail de verificação do Firebase (`noreply@<project-id>.firebaseapp.com`) caiu na caixa de spam do Gmail nos testes, não na caixa de entrada principal. Vale considerar configurar um remetente/domínio personalizado no Firebase Auth (ou pelo menos avisar a família na tela de configurações que o e-mail pode cair no spam) para não fazer a família achar que a verificação não foi enviada.
