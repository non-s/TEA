...
Na trilha da criança, um cartão "Agora" destaca a próxima atividade disponível com um botão "Continuar". A tela mantém aberto apenas o módulo em foco ou revisão leve; os demais módulos ficam resumidos com progresso e podem ser abertos sob demanda ("Ver atividades"). Nenhum módulo fica bloqueado ou marcado com cadeado — a ordem sugerida existe só como recomendação visual (o cartão "Agora"), nunca como restrição: a criança pode abrir qualquer módulo e ir direto na atividade que acha que consegue fazer, sem precisar dominar o módulo anterior primeiro. Assim a navegação livre continua acessível, mas a criança não encontra dezenas de escolhas simultâneas quando precisa apenas saber o próximo passo.

O botão para trocar de perfil não troca a tela imediatamente: ele abre uma confirmação curta, com foco inicial em "Continuar na trilha", e só libera a troca quando o adulto digita `ADULTO`. A intenção é prevenir toque acidental e transição surpresa para a criança, mesmo sem existir conta ou login por trás.
...
Essa previsibilidade começa antes mesmo da primeira atividade: como não há conta nem login, a família chega direto na tela de escolher/criar um perfil — sem formulário de cadastro, senha ou etapa de confirmação para simplesmente conhecer a plataforma.
...
Na interface da criança, o painel de comunicação e regulação oferece cartões como "Pausa", "Ajuda", "Não sei" e "Pronto". Cada ação tem um símbolo visual estável além do texto, com ações seguras e consistentes (por exemplo: o cartão de pausa sempre abre a tela calma de pausa). Quando o perfil tem um interesse escolhido, o painel acrescenta um motivo visual pequeno ligado a esse interesse, sem trocar o símbolo funcional principal nem o nome acessível do cartão. Ele não tenta substituir uma CAA individualizada prescrita por fonoaudiólogo, mas cria uma ponte mínima de comunicação funcional dentro da atividade: uma criança não verbal, minimamente verbal, em sobrecarga ou em fase inicial de alfabetização pode comunicar necessidade sem depender de fala oral.
...
### Apoio preferencial molda a mediação, não é vigilância

O apoio preferencial escolhido na criação do perfil (comunicação, forma de acesso, regulação) altera a experiência da criança: a tela de preparação e a tela de pausa mostram passos compatíveis com o combinado do perfil (visual, fala curta, modelo gestual ou pausa combinada). A habilidade ensinada e os critérios de domínio não mudam; muda apenas a forma de mediar a entrada e a regulação durante a atividade.

Essa escolha segue a lógica de práticas mediadas por tecnologia descritas pelo AFIRM/NCAEP: escolher objetivo, preparar apoios adicionais quando necessário (apoio visual, análise de tarefa, modelagem), treinar a equipe/família e monitorar a resposta do aprendiz. A plataforma não substitui avaliação clínica ou plano terapêutico, mas torna o uso cotidiano mais fiel a necessidades individuais — sem gerar um relatório, ranking ou histórico de "desempenho" para um adulto revisar depois. O único registro que existe é o necessário para a própria trilha funcionar (nível de dica, atividades dominadas), guardado só no aparelho.
...
- Relatório de progresso, exportação de dados e compartilhamento com terapeutas/segundo responsável — o app teve essas telas em versões anteriores; foram removidas deliberadamente na migração para o modelo sem conta (ver `docs/ARQUITETURA.md`), porque exigiriam voltar a ter uma conta/servidor guardando dado da criança. Editar o perfil de apoio depois da criação também não está disponível hoje pelo mesmo motivo: não há mais uma tela de "gerenciar perfis".
