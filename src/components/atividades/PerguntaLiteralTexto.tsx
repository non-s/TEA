...
export function PerguntaLiteralTexto({
  ...
  const pergunta = atividade.pergunta ?? 'O que apareceu no texto?'
  const instrucao = `Leia o texto. Depois responda: ${pergunta} ${texto}`
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-8">
        <section
          aria-label="Texto e pergunta"
          className="flex w-full max-w-md flex-col gap-4 text-center"
        >
          <p className="whitespace-pre-line rounded-3xl border-2 border-[var(--cor-borda)] bg-[var(--cor-fundo-alt)] px-5 py-5 text-2xl leading-10 font-semibold text-[var(--cor-texto)]">
            {atividade.alvo.rotulo}
          </p>
          <p className="rounded-2xl bg-[var(--cor-primaria-clara)] px-5 py-4 text-xl leading-8 font-semibold text-[var(--cor-primaria-escura)]">
            {pergunta}
          </p>
        </section>
