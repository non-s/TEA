...
- Trilha com **492 atividades em 11 módulos**: emparelhamento visual → maiúscula/minúscula → nomeação receptiva de letras → nomeação expressiva de letras (por toque ou por voz opcional) → traçado de letras num guia pontilhado (módulo paralelo, não bloqueia os seguintes) → formação de sílabas CV (15 consoantes de alta frequência × 5 vogais, os dígrafos CH/LH/NH, os encontros consonantais com R e alguns com L — BR/CR/FR/TR/DR/GR/PR, BLO/BLU/CLA/CLO/CLU/FLO/GLO/PLA/PLU — e um primeiro grupo de ditongos orais combinados com consoante, como FEI/BAI/NOI/AU) com apoio de palavra familiar (ex: "MA, de mamãe", "CHA, de chave", "NOI, de noite") → formação de palavras simples com duas sílabas conhecidas → leitura de frases curtas por seleção → compreensão literal de frase → compreensão literal de texto curto com duas frases → perguntas sobre o texto (literal, presença/ausência e inferência guiada, misturadas no mesmo módulo).
